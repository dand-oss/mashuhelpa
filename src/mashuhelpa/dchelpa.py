"""
dc_helpa resolves types dataclasses with string annotations

ie not forward declared
...
def resolve_types(
    a_dc: typing.Any, globalns: typing.Any = None, localns: typing.Any = None
) -> bool:
    """Relove dataclass field types from string version"""
    try:
        # resolving is expensive, so check if already done
        return getattr(a_dc, f"__attrs_types_{a_dc.__name__}_resolved__")

    except AttributeError:
        # resolve  all the string annotations and forward annotations
        # BY EVAL - slow, and requires full context
        hint_dict = typing.get_type_hints(a_dc, globalns=globalns, localns=localns)
